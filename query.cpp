...
int main(int argc, char** argv){
	...
	//Statring the query loop for answering the query
	//hashing the name to number
	map<string,int> symbol_map;
	for(unsigned int i=0;i<symbol_table.size();i++){
		string name=get<1>(symbol_table[i]);
		symbol_map[name]=i;
	}
	regex break_pattern("(.*),(.*)");
	string query;
	cout<<"Initialization Complete!!"<<endl<<endl;
	cout<<"Please Enter your Query and Press Enter\n";
	while(1){
		getline(cin,query);
		smatch matcher;
		regex_search(query,matcher,break_pattern);
		string s1=(string)matcher[1];
		string s2=(string)matcher[2];

		int var_num1=symbol_map[s1];
		int var_num2=symbol_map[s2];
		//cout<<var_num1<<","<<var_num2<<endl;
		if(name_equi_table[var_num1][var_num2]==1){
			cout<<"name equivalent,";
		}
		if(internal_equi[var_num1][var_num2]==1){
			cout<<"internal_equivalent,";
		}
		if(stru_equi_table[var_num1][var_num2]==1){
			cout<<"and structural equivalent";
		}
		cout<<endl<<endl;
	}
